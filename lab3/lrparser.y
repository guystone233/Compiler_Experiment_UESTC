...
CompUnits: CompUnit                      {$$ = newNextNode("CompUnit", $1, NULL); node = $$;}
	| CompUnits CompUnit             {$$ = newNextNode("CompUnit", $1, $2); node = $$;}
	;
...
Stmt: LVal '=' Exp ';'                          {$$ = newBasicNode("Assign_Stmt", $1, $3, NULL);}
      ...
      | Block                                   {$$ = $1;}
      | IF '(' Cond ')' Stmt %prec IFX          {$$ = newBasicNode("If_Stmt", $3, $5, NULL);}
...
Number: num_INT                {$$ = newNum(yylval.iVal);}
      ;
